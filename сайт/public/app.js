...
const REMOTE_CONTENT_KEYS = new Set([
  STORAGE_KEYS.about,
  STORAGE_KEYS.portfolio,
  STORAGE_KEYS.hero,
  STORAGE_KEYS.animators,
  STORAGE_KEYS.users,
  STORAGE_KEYS.orders,
...
function setJson(key, value) {
  ...
  if (REMOTE_CONTENT_KEYS.has(key)) {
    saveRemoteStateKey(key, value);
  }
}
...
async function resolveImageValue(file, options = {}) {
  if (!file || !file.size) return null;
  const { maxWidth = 1800, quality = 0.9, maxBytes = 380000, preferUpload = true } = options;
  if (preferUpload) {
    try {
      const uploadedPath = await uploadImageFile(file);
      if (uploadedPath) return uploadedPath;
    } catch {
      // Fallback to local data URL when upload API is unavailable.
    }
  }
...
async function loadRemoteState() {
  try {
    ...
    if (!response.ok || !payload?.state || typeof payload.state !== "object") return;
    for (const key of REMOTE_CONTENT_KEYS) {
      if (Object.prototype.hasOwnProperty.call(payload.state, key)) {
        localStorage.setItem(key, JSON.stringify(payload.state[key]));
      } else {
        const localRaw = localStorage.getItem(key);
        if (!localRaw) continue;
        try {
          const localValue = JSON.parse(localRaw);
          await saveRemoteStateKey(key, localValue);
        } catch {
          // Skip corrupted local JSON entries.
        }
      }
    }
  } catch {
    // Offline/local fallback: keep current localStorage values.
  }
}

async function saveRemoteStateKey(key, value) {
  try {
    ...
    // No-op: localStorage still keeps local backup.
  }
}
